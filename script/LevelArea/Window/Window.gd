...
@export (NodePath) onready var _body = get_node(_body) as AnimatedSprite2D
@export (NodePath) onready var _border = get_node(_border) as Sprite2D
@export (NodePath) onready var _tip = get_node(_tip) as Sprite2D
@export (NodePath) onready var _animation_player = get_node(_animation_player) as AnimationPlayer
@export (NodePath) onready var _ligth = get_node(_ligth) as PointLight2D
...
func _ready() -> void:
	EventStorage.connect("steped", Callable(self, "_on_steped"))
	EventStorage.connect("steped_back", Callable(self, "_on_steped_back"))
	EventStorage.connect("reseted", Callable(self, "_on_reseted"))
