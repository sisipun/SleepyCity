...
@export (NodePath) onready var _house = get_node(_house) as Sprite2D
@export (NodePath) onready var _roof = get_node(_roof) as Sprite2D
...
func _ready() -> void:
	...
	EventStorage.connect("decrement_tip_request", Callable(self, "_on_decrement_tip_request"))
	EventStorage.connect("reset_request", Callable(self, "_on_reset_request"))
	EventStorage.connect("step_back_request", Callable(self, "_on_step_back_request"))
...
func add_window(i: int) -> void:
	var window: Window = _window_scene.instantiate()
	add_child(window)
